...
CmdResult CommandFilter::Handle(const std::vector<std::string> &parameters, User *user)
{
	...
	else
	{
		...
		if (parameters.size() >= 4)
		{
			...
			{
				user->WriteServ("NOTICE %s :*** Invalid filter type '%s'. Supported types are 'gline', 'log', 'none', 'block', 'silent' and 'kill'.", user->nick.c_str(), parameters[1].c_str());
				return CMD_FAILURE;
			}
...
ModResult ModuleFilter::OnUserPreNotice(User* user,void* dest,int target_type, std::string &text, char status, CUList &exempt_list)
{
	...
	if (f)
	{
		...
		else if (f->action == FA_GLINE)
		{
			...
			ServerInstance->SNO->WriteGlobalSno('a', "FILTER: "+user->nick+" was banned, target was "+target+" (Reason: "+f->reason+") Text: "+text);
